...
export class TransformCropEngine {
     ...
     * @param options - 輸出選項
     */
    static async process(image, params, options = {}) {
        const { outputFormat = 'png', quality = 0.95, smoothing = true, backgroundColor = '#ffffff' } = options;
        // 計算實際裁切區域（自然座標）
        const actualCropRect = CoordinateUtils.displayToNatural(params.cropRect, params.naturalSize, params.displaySize, params.displayOffset);
        // 計算輸出尺寸（基於顯示座標的裁切區域）
        ...
        ctx.translate(displayCenterX + params.transform.tx, displayCenterY + params.transform.ty);
        // 4) 套用旋轉和縮放
        ...
        const dataURL = canvas.toDataURL(outputFormat === 'png' ? 'image/png' : `image/${outputFormat}`, quality);
        return {
            blob,
            dataURL,
     ...
     * 批次處理多張圖片
     */
    static async processMultiple(images, paramsList, options = {}) {
        const results = [];
...
//# sourceMappingURL=transform-crop-engine.js.map