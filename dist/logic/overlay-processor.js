...
import { TransformCropEngine } from './image-processor/index.js';
/**
...
export class OverlayProcessor {
    ...
    static async processOverlay(overlay, options = {}) {
        ...
        // 3. 設置虛擬顯示參數（用於座標計算）
        const displaySize = { w: naturalSize.w, h: naturalSize.h };
    ...
    static needsHighQualityProcessing(overlay) {
        ...
        if (Math.abs(overlay.rotation) > 0.01)
            return true;
        // 有縮放（非 1:1）
        if (Math.abs(overlay.scaleX - 1) > 0.01 || Math.abs(overlay.scaleY - 1) > 0.01)
            return true;
        // 有裁切
        if (overlay.crop.x > 0 || overlay.crop.y > 0 ||
            ...
            return true;
        // 透明度不是完全不透明
        if (overlay.opacity < 0.99)
            return false; // 透明度不影響是否需要高品質處理
        return false;
    }
    /**
     * 創建圖層預覽（低品質，用於即時預覽）
     ...
     */
    static createPreview(overlay, previewSize = 200) {
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        // 計算預覽尺寸
        ...
        // 繪製裁切區域
        ctx.drawImage(overlay.img, overlay.crop.x, overlay.crop.y, overlay.crop.w, overlay.crop.h, -overlay.crop.w / 2, -overlay.crop.h / 2, overlay.crop.w, overlay.crop.h);
        ctx.restore();
        return canvas;
    }
    /**
