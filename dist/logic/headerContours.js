export const headerContourIds = [
    'soft-wave',
    'arc-sweep',
    'layered-ribbon',
    'clean-diagonal'
];
export const headerContourLabels = {
    'soft-wave': '免疫訊號',
    'arc-sweep': '精準辨識',
    'layered-ribbon': '協同網絡',
    'clean-diagonal': '免疫級聯'
};
const HEADER_BOUNDARY_KNOTS = {
    'soft-wave': [
...
export function getHeaderContourBoundarySamples(contourId) {
    return HEADER_BOUNDARY_KNOTS[contourId];
}
export function createContourGradient(ctx, colors, width, height) {
...
function traceBoundaryRightToLeft(ctx, points, width, height) {
    const tangents = getSmoothTangents(points);
    for (let index = points.length - 2; index >= 0; index -= 1) {
        const left = points[index];
        const right = points[index + 1];
        const dx = right.x - left.x;
        ctx.bezierCurveTo((right.x - dx / 3) * width, (right.y - tangents[index + 1] * dx / 3) * height, (left.x + dx / 3) * width, (left.y + tangents[index] * dx / 3) * height, left.x * width, left.y * height);
    }
}
function traceBoundaryLeftToRight(ctx, points, width, height) {
    const tangents = getSmoothTangents(points);
    for (let index = 0; index < points.length - 1; index += 1) {
        const left = points[index];
        const right = points[index + 1];
        const dx = right.x - left.x;
        ctx.bezierCurveTo((left.x + dx / 3) * width, (left.y + tangents[index] * dx / 3) * height, (right.x - dx / 3) * width, (right.y - tangents[index + 1] * dx / 3) * height, right.x * width, right.y * height);
    }
}
...
function fillOpticalSurface(ctx, alpha, trace) {
    ctx.save();
...
function strokeLightPath(ctx, width, alpha, lineWidth, trace) {
    ctx.save();
    ctx.globalAlpha = alpha;
...
function drawSignalNode(ctx, width, height, x, y, radius) {
    const scale = width / 800;
    const gradient = ctx.createRadialGradient(x * width, y * height, 0, x * width, y * height, radius * scale);
...
function drawSignalResonance(ctx, width, height) {
...
function drawRefractiveFocus(ctx, width, height) {
...
function drawOrganicFacets(ctx, width, height) {
    ...
    const titleMist = ctx.createRadialGradient(width * 0.50, height * 0.43, 0, width * 0.50, height * 0.43, width * 0.46);
...
function drawCascadeField(ctx, width, height) {
...
function drawTransitionBands(ctx, contourId, width, height) {
    [
...
export function renderContourPreview(canvas, contourId, colors) {
...
//# sourceMappingURL=headerContours.js.map