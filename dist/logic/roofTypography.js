...
export function getRoofTitleInk(styleId, colors, material) {
    const pixels = material.getContext('2d').getImageData(Math.floor(material.width * .3), Math.floor(material.height * .1), Math.floor(material.width * .4), Math.floor(material.height * .23)).data;
...
export function getRoofAgendaInk(colors, position, darkInk) {
    const t = Math.max(0, Math.min(1, position)) * (colors.length - 1);
    const i = Math.min(colors.length - 2, Math.floor(t)), f = t - i;
    const rgb = [1, 3, 5].map(offset => (parseInt(colors[i].slice(offset, offset + 2), 16) * (1 - f) + parseInt(colors[i + 1].slice(offset, offset + 2), 16) * f) / 255);
    ...
    for (let step = 0; step < 48; step++) {
        ...
        if (contrastRatio(backgroundL, luminance(quantized)) >= 4.8) {
            darkRGB = quantized;
            break;
        }
...
//# sourceMappingURL=roofTypography.js.map