...
export class CropController {
  ...
  private cropTouchStart: (e: TouchEvent) => void;
  private cropTouchMove: (e: TouchEvent) => void;
  private cropTouchEnd: (e: TouchEvent) => void;
  ...
  private onCropTouchMove(e: TouchEvent): void {
    if (!this.cropState.isActive) return;
    e.stopImmediatePropagation();
    if (!this.cropState.isDragging || !this.cropState.dragHandle || e.touches.length !== 1) return;
   ...
   * 裁切推桿碰撞檢測 - 修復推桿位置計算
   */
  private cropHitTest(point: { x: number; y: number }, hitRadius = 15): { hit: boolean; handle: string } {
  ...
  private canvasPixelsForCssPixels(cssPixels: number): number {
    const rect = this.canvas.getBoundingClientRect();
