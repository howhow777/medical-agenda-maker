...
import { RoofSelectionStore, recommendedRoofSelection } from '../logic/roofSelection.js';
...
export class RoofStyleControls {
  private cancerId: CancerDesignPresetId = 'lung';
  private coordinator = new RoofLoadCoordinator(roofMaterialLibrary);
  private status = '';
  private retryable = false;
  private hosts = Array.from(document.querySelectorAll<HTMLElement>('[data-roof-style-host]'));

  constructor(
    public readonly store: RoofSelectionStore,
    private form: FormControls,
    private onChange: (cancerId: CancerDesignPresetId, selection?: RoofSelection) => void
  ) {
    ['headerC1', 'headerC2', 'headerC3', 'agendaBg', 'agendaBorder', 'agendaAccent', 'bgC1', 'bgC2'].forEach(id => {
      document.getElementById(id)?.addEventListener('input', () => {
        if (!this.store.get(this.cancerId) || this.isRestoring()) return;
        ...
        for (const key of ['headerC1', 'headerC2', 'headerC3', 'agendaBg', 'agendaBorder', 'agendaAccent', 'bgC1', 'bgC2'] as const) {
  ...
  async setCancer(cancerId: CancerDesignPresetId): Promise<void> {
    this.cancerId = cancerId;
    const selection = this.store.get(cancerId);
    if (selection) this.applySelectionColors(selection);
    this.render();
    await this.loadSelection(selection);
  }

  async select(styleId: RoofStyleId): Promise<void> {
  ...
  syncCustomColors(): void {
    ...
    if (JSON.stringify(current) === JSON.stringify(selection)) return;
    this.store.select(this.cancerId, selection);
    if (selection.mode === 'recommended') this.applySelectionColors(selection);
    this.updateControls();
    void this.loadSelection(selection);
  }

  async readyForExport(): Promise<void> { await this.coordinator.readyForExport(); }

  private isRestoring(): boolean { return document.documentElement.dataset.restoringAgenda === 'true'; }

  private applySelectionColors(selection: RoofSelection): void {
  ...
  private async loadSelection(selection?: RoofSelection): Promise<void> {
  ...
  private render(): void {
    ...
    const focusHost = focused?.closest<HTMLElement>('[data-roof-style-host]');
    ...
    this.hosts.forEach((host, index) => {
      const heading = document.createElement('h3'); heading.textContent = `${preset.label}｜核准屋簷`;
      const grid = document.createElement('div'); grid.className = 'roof-style-grid';
      grid.setAttribute('role', 'group'); grid.setAttribute('aria-label', `${preset.label}屋簷款式`);
      getRoofStylesForCancer(this.cancerId).forEach(style => {
        const button = document.createElement('button'); button.type = 'button'; button.className = 'roof-style-card';
        button.dataset.roofStyleId = style.id; button.setAttribute('aria-label', `套用${style.label}`);
        const preview = document.createElement('canvas'); preview.width = 240; preview.height = Math.ceil(getRoofPlacement(style.id, 240).height);
        preview.setAttribute('aria-hidden', 'true');
        const label = document.createElement('span'); label.textContent = style.label;
        button.append(preview, label); button.addEventListener('click', () => { void this.select(style.id); });
        grid.append(button);
      });
      const modes = document.createElement('div'); modes.className = 'roof-color-modes';
      for (const [action, label] of [['recommended', '恢復建議配色'], ['custom', '自訂三色'], ['legacy', '使用原輪廓']] as const) {
        const button = document.createElement('button'); button.type = 'button'; button.dataset.roofAction = action;
        button.textContent = label;
        button.addEventListener('click', () => {
          const selection = this.store.get(this.cancerId);
          if (action === 'legacy') { void this.useLegacy(); return; }
          if (!selection) return;
          if (action === 'recommended') { void this.select(selection.styleId); return; }
          this.form.setCurrentColorScheme('custom'); this.syncCustomColors();
        });
        modes.append(button);
      }
      const colors = document.createElement('div'); colors.className = 'roof-custom-colors';
      ['深層', '膜面', '透光'].forEach((label, channel) => {
        const wrap = document.createElement('label'); wrap.textContent = label;
        const input = document.createElement('input'); input.type = 'color'; input.dataset.roofChannel = String(channel);
        input.setAttribute('aria-label', `${label}色彩`);
        input.addEventListener('change', () => {
          const current = this.form.getCustomColors();
          const key = ['headerC1', 'headerC2', 'headerC3'][channel] as 'headerC1' | 'headerC2' | 'headerC3';
          this.form.setCustomColors({ ...current, [key]: input.value });
          this.form.setCurrentColorScheme('custom'); this.syncCustomColors();
        });
        wrap.append(input); colors.append(wrap);
      });
      const notice = document.createElement('p'); notice.className = 'roof-color-notice'; notice.dataset.roofNotice = '';
      const status = document.createElement('p'); status.dataset.roofStatus = ''; status.setAttribute('role', 'status'); status.setAttribute('aria-live', 'polite');
      const retry = document.createElement('button'); retry.type = 'button'; retry.textContent = '重試載入屋簷'; retry.dataset.roofRetry = '';
    ...
    this.updateControls();
    if (focusHost && focusKey) focusHost.querySelector<HTMLElement>(focusKey)?.focus();
  }

  private updateControls(): void {
    const selection = this.store.get(this.cancerId);
    const recommendedOption = document.querySelector<HTMLOptionElement>('#colorScheme option[value="optical_recommended"]');
    if (recommendedOption) recommendedOption.disabled = !selection;
    this.hosts.forEach(host => {
      host.querySelectorAll<HTMLButtonElement>('[data-roof-style-id]').forEach(button => {
        const styleId = button.dataset.roofStyleId as RoofStyleId;
        button.setAttribute('aria-pressed', String(selection?.styleId === styleId));
        const canvas = button.querySelector('canvas')!;
        const colors = selection?.styleId === styleId ? selection.colors : getRecommendedRoofScheme(this.cancerId, styleId).header.colors as RoofColors;
        ...
        }).catch(() => { if (canvas.isConnected) { ctx.fillStyle = '#475569'; ctx.font = '14px sans-serif'; ctx.fillText('素材未載入', 12, 38); } });
      });
      host.querySelectorAll<HTMLButtonElement>('[data-roof-action]').forEach(button => {
      ...
      host.querySelectorAll<HTMLInputElement>('[data-roof-channel]').forEach(input => {
        input.disabled = !selection;
        input.value = selection?.colors[Number(input.dataset.roofChannel)] || '#FFFFFF';
      });
      const note = host.querySelector<HTMLElement>('[data-roof-notice]')!;
  ...
  private updateStatus(): void {
    this.hosts.forEach(host => {
      const status = host.querySelector<HTMLElement>('[data-roof-status]');
      if (status) status.textContent = [this.status, ...this.store.issues].filter(Boolean).join(' ');
      const retry = host.querySelector<HTMLButtonElement>('[data-roof-retry]');
      if (retry) retry.hidden = !this.retryable;
    });
  }
}
