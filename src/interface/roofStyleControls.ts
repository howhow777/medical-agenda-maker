...
import {
  isOpticalRoofSelection,
  recommendedClassicRoofSelection,
  recommendedRoofSelection,
  RoofSelectionStore
} from '../logic/roofSelection.js';
...
export class RoofStyleControls {
  ...
  async selectClassic(): Promise<void> {
  ...
  async readyForExport(): Promise<void> {
  ...
  private render(): void {
    ...
    this.hosts.forEach((host, index) => {
      ...
      for (const [action, label] of [['recommended', '恢復建議配色'], ['custom', '自訂三色']] as const) {
        const button = document.createElement('button');
        button.type = 'button';
        ...
        button.addEventListener('click', () => {
          ...
          if (action === 'recommended') {
            if (isOpticalRoofSelection(selection)) void this.select(selection.styleId);
            else void this.selectClassic();
            return;
          }
          this.form.setCurrentColorScheme('custom');
          this.syncCustomColors();
  ...
  private updateControls(): void {
    ...
    if (recommendedOption) recommendedOption.disabled = !optical;
    ...
    this.hosts.forEach(host => {
      host.querySelectorAll<HTMLButtonElement>('[data-roof-style-id]').forEach(button => {
        ...
        const colors = optical && selection.styleId === styleId
          ? selection.colors
          : getRecommendedRoofScheme(this.cancerId, styleId).header.colors as RoofColors;
      ...
      const classicButton = host.querySelector<HTMLButtonElement>('[data-roof-choice-id="classic"]');
      if (classicButton) {
