...
import { AgendaData, AgendaBasicInfo, AgendaItem, ExcelParseResult } from '../assets/agendaTypes.js';
...
export class ExcelAgendaParser {
   ...
   * 解析基本資訊
   */
  private parseBasicInfo(data: any[][]): AgendaBasicInfo {
    const basicInfo: AgendaBasicInfo = {
      title: '',
      venue: '',
      date: '',
      time: ''
   ...
   * 解析議程項目
   */
  private parseAgendaItems(data: any[][]): AgendaItem[] {
   ...
   * 解析單一議程行
   */
  private parseAgendaRow(row: any[]): AgendaItem | null {
    ...
    // 過濾空行或無效行
    if (!timeStr || !contentStr) return null;
    if (timeStr.toLowerCase().includes('time') || timeStr.includes('時間')) return null;
    ...
    // 判斷議程類型
    let type: AgendaItem['type'] = 'presentation';
   ...
   * 輔助方法：在行中找到值
   */
  private findValueInRow(row: any[], startIndex: number): string {
   ...
   * 計算總時長
   */
  private calculateTotalDuration(items: AgendaItem[]): string {
    if (items.length === 0) return '0 小時';
  ...
  private smartFieldDetection(row: any[]): { speaker: string | undefined; moderator: string | undefined } {
    ...
    if (rowLength <= 5) {
      ...
      if (pos3 && (!speaker || speaker.length < 3)) {
        if (!speaker) speaker = pos3; // 第3位置可能是 Speaker
