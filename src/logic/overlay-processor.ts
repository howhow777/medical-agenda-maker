...
export class OverlayProcessor {
  ...
  static needsHighQualityProcessing(overlay: Overlay): boolean {
    ...
    if (Math.abs(overlay.scaleX - 1) > 0.01) return true;
    if (Math.abs(overlay.scaleY - 1) > 0.01) return true;
   ...
   */
  static createPreview(overlay: Overlay, previewSize: number = 150): HTMLCanvasElement {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d')!;
    ...
    const scale = Math.min(
      previewSize / overlay.w,
      previewSize / overlay.h
    );
