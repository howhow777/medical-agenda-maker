import { AgendaItem, ColorScheme, CancerTemplate, GradientDirection, CustomColors, HeaderContourId, Overlay } from '../assets/types.js';
...
export class PosterRenderer {
  ...
  private drawPresetHeader(_templateId: string, scheme: ColorScheme, W: number, direction: string): void {
    drawHeaderContour(
      this.ctx,
      this.headerContourId,
      W,
      150,
      this.createGradient(W, 150, scheme.header.colors, direction)
    );
  }
  ...
  drawPoster(
    ...
    conferenceData: ConferencePosterData,
    showFooter: boolean,
    footerText: string,
    overlays: Overlay[] = [],
    ...
    if (agendaItems.length > 0) {
      afterAgendaY = this.drawAgendaTable(
        agendaItems,
        scheme,
        W,
        afterAgendaY,
    ...
    while (titleSize > 24) {
      ...
      if (this.ctx.measureText(title).width <= W - 80) break;
      titleSize -= 1;
    }
    this.ctx.font = `bold ${titleSize}px Microsoft JhengHei`;
  ...
  private drawAgendaTable(
    ...
    // 資料行
    const pad = 10;
    const rowLayouts: Array<{ item: AgendaItem; top: number; height: number; index: number }> = [];
    const normalizeModerator = (value: string): string => value.trim().replace(/\s+/g, ' ');
  ...
  private calculateAgendaRowHeights(agendaItems: AgendaItem[], W: number, hideModerator: boolean): number[] {
  ...
  private drawOverlaysClippedToHeader(overlays: Overlay[], W: number): void {
    this.ctx.save();
    this.ctx.beginPath();
    traceHeaderContourPath(this.ctx, this.headerContourId, W, 150);
    this.ctx.clip();
   ...
   */
  private getConferenceDataFromDOM(): ConferencePosterData {
    return {
