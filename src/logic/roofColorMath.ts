...
export const linear = (v: number): number => v <= .04045 ? v / 12.92 : ((v + .055) / 1.055) ** 2.4;
const srgb = (v: number): number => v <= .0031308 ? 12.92 * v : 1.055 * v ** (1 / 2.4) - .055;
...
export function labToRGB([L, a, b]: Triple): Triple {
...
export const hexLab = (hex: string): Triple => rgbToLab([1, 3, 5].map(i => parseInt(hex.slice(i, i + 2), 16) / 255) as Triple);

export function tonalWeights(L: number, anchors: Triple[]): Triple {
  if (L < anchors[1][0]) {
...
export function transferRoofChroma(a: number, b: number, baseA: number, baseB: number, targetA: number, targetB: number): [number, number] {
  const denominator = baseA * baseA + baseB * baseB;
  const targetC = Math.hypot(targetA, targetB);
  if ((a === 0 && b === 0) || denominator < 1e-12 || targetC < 1e-6) return [0, 0];
  ...
  if (targetHue > Math.PI / 3 && targetHue < Math.PI * 2 / 3) {
    if (baseA === targetA && baseB === targetB) return [a, b];
    ...
    outA = chroma * Math.cos(hue); outB = chroma * Math.sin(hue);
  }
  return [outA, outB];
}

export interface RoofMaps { width: number; height: number; values: Float32Array; }
...
export function buildRoofMaps(pixels: Uint8ClampedArray, width: number, height: number, sourceColors: RoofColors): RoofMaps {
  ...
  for (let p = 0; p < pixels.length / 4; p++) {
    const rgb: Triple = [pixels[p * 4] / 255, pixels[p * 4 + 1] / 255, pixels[p * 4 + 2] / 255];
...
export function recolorRoofPixels(maps: RoofMaps, sourceColors: RoofColors, colors: RoofColors): Uint8ClampedArray {
  const anchors = sourceColors.map(hexLab);
  ...
  for (let p = 0; p < pixels.length / 4; p++) {
    ...
    const lab: Triple = [values[k], values[k + 1], values[k + 2]];
    ...
    for (let j = 0; j < 3; j++) pixels[p * 4 + j] = alpha > 0 ? clamp((rgb[j] - 1 + alpha) / alpha) * 255 : 0;
